...
export default function AdminProjects() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      // Remove empty optional fields so we don't send empty strings to date columns
      const payload = { ...formData };
      if (!payload.date_completed) delete payload.date_completed;
      ...
      alert('Gagal menyimpan. Pastikan izin (Policy) Supabase sudah diperbarui.');
      console.error(error);
    } finally {
      setSaving(false);
    }
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-xl relative overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Nama Klien (Opsional)</label>
                  <input type="text" value={formData.client_name} onChange={e => setFormData({...formData, client_name: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Tanggal (Opsional)</label>
                  <input type="date" value={formData.date_completed} onChange={e => setFormData({...formData, date_completed: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary outline-none" />
                </div>
              </div>
