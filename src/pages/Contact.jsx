...
const Contact = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-50 to-slate-100 text-secondary font-bold">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-blue-900/10 border border-white p-8 md:p-12 lg:p-16 flex flex-col lg:flex-row gap-12 lg:gap-16">
          ...
          >
            {(settings?.address || settings?.email || settings?.wa_number) && (
              <h3 className={`text-2xl md:text-3xl font-extrabold text-secondary mb-10 flex items-center gap-3 justify-center ${settings?.map_embed_url ? 'lg:justify-start' : ''}`}>
                <Building2 className="w-8 h-8 text-primary hidden lg:block" />
                Creative Box
              </h3>
            )}
          ...
          {settings?.map_embed_url && (
            ...
            >
              <div className="absolute inset-0 bg-secondary/5 group-hover:bg-transparent transition-colors duration-500 pointer-events-none z-10" />
              <iframe 
      ...
      <section className="w-full py-20 bg-secondary relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-12">
            <h3 className="text-primary font-bold tracking-widest uppercase mb-2">Tinggalkan Pesan</h3>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <input 
                ...
                className="w-full px-5 py-4 bg-white/90 text-foreground rounded-xl outline-none focus:ring-2 focus:ring-primary transition-all font-medium"
              />
              <input 
                type="email" 
                required
            ...
            <div className="mb-8">
              <textarea 
                ...
                rows="5"
                className="w-full px-5 py-4 bg-white/90 text-foreground rounded-xl outline-none focus:ring-2 focus:ring-primary transition-all font-medium resize-none"
              ></textarea>
            </div>
            <button 
              type="submit"
              disabled={formStatus === 'sending...'}
              className="w-full bg-gradient-to-r from-primary to-primary hover:from-yellow-400 hover:to-orange-500 text-white font-extrabold py-4 px-8 rounded-xl shadow-lg shadow-orange-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {formStatus === 'sending...' ? (
